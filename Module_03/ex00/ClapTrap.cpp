...
ClapTrap::ClapTrap(std::string name): name_(name), hitPoints_(10), energyPoints_(10), attackDamage_(0){
	std::cout << "Constructor of ClapTrap called: " << this->name_ << " created" << std::endl;
}

ClapTrap::ClapTrap(const ClapTrap &copy){
	std::cout << "Copy Constructor of ClapTrap called " << this->name_ << std::endl;
	*this = copy;
}
...
void ClapTrap::takeDamage(unsigned int amount){
	if (this->hitPoints_ == 0)
		std::cout << this->name_ << " is dead and can not take further damage" << std::endl;
	else if (this->hitPoints_ <= amount){
		std::cout << this->name_ << " took " << hitPoints_ << " damage and died" << std::endl;
		this->hitPoints_ = 0;
	} else {
		this->hitPoints_ -= amount;
		std::cout << this->name_ << " took " << amount << " damage" << std::endl;
	}
}

void ClapTrap::beRepaired(unsigned int amount){
	if (this->energyPoints_ == 0)
		std::cout << this->name_ << " has no energy points and can not repair" << std::endl;
	else if (this->hitPoints_ == 0)
		std::cout << this->name_ << " is dead and can not repair" << std::endl;
	else {
		this->hitPoints_ = this->hitPoints_ + amount;
		this->energyPoints_--;
		std::cout << this->name_ << " spent 1 energy to repair and receives " << amount << " hit points" << std::endl;
	}
}
